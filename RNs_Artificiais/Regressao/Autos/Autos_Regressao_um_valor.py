...
## Criação da variável tipo dummy (parecido com a transformação de um número em um número binário)
# onehot_encoder = OneHotEncoder(categorical_features = [0, 1, 3, 5, 8, 9, 10])
## Variável previsores recebe o onehot_encoder e armazena em uma matriz
# previsores = onehot_encoder.fit_transform(previsores).toarray()


""" Linhas de código atualizadas para a criação da variável do tipo dummy (encontrado na internet) """
# ColumnTransformer (matriz chamada de transformadores, remainder)
# Matriz Transformadores: [(um nome para o transformador, estimador (sparce=False para array), linhas ou colunas a serem transformadas)]
# Remainder: dirá ao transformador o que fazer com as outras colunas. "passthroungh" sigifica mante-lás sem alteração
ct = ColumnTransformer([('encoder', OneHotEncoder(sparse=False), [0, 1, 3, 5, 8, 9, 10])], remainder='passthrough')
# Ajustar e transformar o conjuntos de dados para a labelencoder e onehotencolder
